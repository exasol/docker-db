    ...
    def start_cluster(self, cmd=None, auto_remove=False, dummy_mode=False, wait=False, wait_timeout=None):
        ...
        if not dummy_mode:
            ...
            if self.merge_exaconf(allow_self = True, force = False) is True:
                ...
                for n,volume in node_volumes.items():
                    internal_exaconf_path = os.path.join(volume, self.exaconf.etc_dir)
                    internal_license_path = os.path.join(volume, self.exaconf.etc_dir, self.exaconf.license_filename)
                    shutil.copy(conf_path, internal_exaconf_path)
                    shutil.copy(license, internal_license_path)
                    # now fix the license path in the INTERNAL EXAConf
                    internal_exaconf = EXAConf.EXAConf(internal_exaconf_path, initialized = True)
                    internal_exaconf.set_license_file(os.path.join(self.exaconf.container_root,
                                                                   self.exaconf.etc_dir,
                                                                   self.exaconf.license_filename),
                                                      commit = True)
